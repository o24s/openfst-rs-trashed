...
fn build_test_fst() -> StdVectorFst {
    ...
    // Epsilons (0 = EPS)
    fst.add_arc(s0, StdArc::new(0, 0, TropicalWeight(1.0), s1)); // 完全Epsilon
    fst.add_arc(s0, StdArc::new(0, 5, TropicalWeight(2.0), s1)); // Input Epsilon
    fst.add_arc(s0, StdArc::new(5, 0, TropicalWeight(3.0), s1)); // Output Epsilon
